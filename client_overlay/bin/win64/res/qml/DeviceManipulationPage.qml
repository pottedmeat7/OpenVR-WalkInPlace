...
import pottedmeat7.walkintplace 1.0
...
MyStackViewPage {
    ...
    content: ColumnLayout {
        ...
        RowLayout {
            ...
            MyComboBox {
                ...
                Layout.fillWidth: true
                model: ["Default", "Disable", "Redirect to", "Swap with", "Motion Compensation", "Step Detection"]
                onCurrentIndexChanged: {
        ...
        RowLayout {
            ...
            MyPushButton {
                ...
                onClicked: {
                    if (deviceSelectionComboBox.currentIndex >= 0) {
                        motionCompensationPage.setDeviceIndex(deviceSelectionComboBox.currentIndex)
                        MyResources.playFocusChangedSound()
        ...
        RowLayout {
            ...
            MyPushButton {
                id: deviceManipulationStepDetectButton
                enabled: true
                activationSoundEnabled: false
                Layout.preferredWidth: 548
                text: "Step Detection"
                onClicked: {
                    MyResources.playFocusChangedSound()
                    stepDetectionPage.updateInfo()
                    var res = mainView.push(stepDetectionPage)
                }
            }
        }
    ...
    function fetchDeviceInfo() {
        ...
        if (index >= 0) {
            ...
            } else if (deviceMode == 6) { // step detection
                if (deviceState == 0) {
                    statusText = "Step Detection"
                } else {
                    statusText = "Step Detection (Unknown state " + deviceState.toString() + ")"
                }
            } else {
