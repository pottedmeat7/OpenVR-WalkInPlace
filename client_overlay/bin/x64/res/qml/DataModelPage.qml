            ...
            GridLayout {
                ...
                Canvas {
                    ...
                    onPaint: {
                        ...
                        var graphXScale = ((rectWidth-topX) > hmdXPoints.length ? ((rectWidth-topX)/hmdXPoints.length) : (hmdXPoints.length/(rectWidth-topX)));
                        ...
                        if ( showHMD.checked ) {
                            ...
                            var lastX = 50;
                            for(var x=0; x<hmdXPoints.length; x++) {
                                ctx.beginPath();                
                        ...
                        if ( showControllers.checked ) {
                            ...
                            var lastX = 50;
                            for(var x=0; x<cont1XPoints.length; x++) {
                                ctx.beginPath();                
                                ctx.strokeStyle = "#DD0000";
                                ctx.lineWidth = 1;
                        ...
                        if ( showTrackers.checked ) {
                            ...
                            var lastX = 50;
                            for(var x=0; x<tracker1XPoints.length; x++) {
                                ctx.beginPath();                
                                ctx.strokeStyle = "#DD0000";
                                ctx.lineWidth = 1;
    ...
    function getModelData() {
        try {
            ...
            while(i<velVals.length) {
                if ( (i+2) < velVals.length ) {
                    var velX = parseFloat(velVals[i+0]).toFixed(6);
                    var velY = parseFloat(velVals[i+1]).toFixed(6);
                ...
                if ( (i+8) < velVals.length ) {
                    velX = parseFloat(velVals[i+6]).toFixed(6);
                    velY = parseFloat(velVals[i+7]).toFixed(6);
                ...
                if ( (i+11) < velVals.length ) {
                    velX = parseFloat(velVals[i+9]).toFixed(6);
                    velY = parseFloat(velVals[i+10]).toFixed(6);
                ...
                if ( (i+14) < velVals.length ) {
                    velX = parseFloat(velVals[i+12]).toFixed(6);
                    velY = parseFloat(velVals[i+13]).toFixed(6);
                ...
                if ( (i+15) < velVals.length ) {
                    var r = (canvasPlotMaxPoint - canvasPlotMinPoint)  
                    var g = (modelCanvas.rectHeight)
                    var finalPoint = (((velVals[i+15] - canvasPlotMinPoint) * g) / r)
                    touchPoints.push(finalPoint);
                    console.info(velVals[i+15]);
                }
                i = i + 16;
            }
