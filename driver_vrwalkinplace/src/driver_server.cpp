...
#define REMOVE_MH_HOOK(detourInfo) \
	if (detourInfo.enabled) { \
		...
		void CServerDriver::setAccuracyButtonAsToggle(bool value) {
			useButtonAsToggle = value;
			g_useButtonAsToggle = value;
		}

		void CServerDriver::setAccuracyButtonFlip(bool value) {
			flipButtonUse = value;
		}
		...
		bool CServerDriver::getStepDetected() {
			return this->_stepPoseDetected;
		}
		...
		bool CServerDriver::_applyStepPoseDetect(vr::DriverPose_t& pose, OpenvrWalkInPlaceInfo* deviceInfo) {
			//_lastposeValid = pose.poseIsValid;
			if (this->_stepPoseDetectEnabled && pose.poseIsValid) {
				...
				if (tdiff >= deltatime) {
					...
					if (!this->_stepPoseDetected) {
						if (isHMD) {
							...
							if (isTakingStep(poseWorldVel, _hmdThreshold, roll, pitch)) { //&& isStepingInPlace(pose.vecPosition) ) {
								if (_openvrDeviceStepPoseTracker[0] == 0) {
									...
									if (velsign != hmdsign) {
										if (velsign < 0) {
											_openvrDeviceStepPoseTracker[0] = -1 * _openvrDeviceStepPoseTracker[0] - 1;
										}
										else {
											_openvrDeviceStepPoseTracker[0] = -1 * _openvrDeviceStepPoseTracker[0] + 1;
										}
								...
								if (_openvrDeviceStepPoseTracker[0] == 0 || (now - _timeLastStepPeak) > 1000) {
							...
							if (_controllerDeviceIds[0] < 0) {
								_controllerDeviceIds[0] = deviceInfo->openvrId();
							}
							else if (_controllerDeviceIds[1] < 0) {
								_controllerDeviceIds[1] = deviceInfo->openvrId();
							}
							if (_controlSelect != 2 && deviceInfo->openvrId() != _controlUsedID) {
						...
						//}
						if (std::abs(_openvrDeviceStepPoseTracker[0]) >= 1) {
					...
					if (this->_stepPoseDetected) {
						...
						if (isHMD) {
							...
							if (isJoggingStep(pose.vecVelocity) || isRunningStep(pose.vecVelocity)) {
								_stepIntegrateSteps = 0;
								_timeLastStepPeak = now;
							}
						}
						...
						&& ((_controlUsedID >= 0 && deviceInfo->openvrId() == _controlUsedID) ||
						(_controlUsedID < 0
							&& (deviceInfo->openvrId() == _controllerDeviceIds[0]
								|| deviceInfo->openvrId() == _controllerDeviceIds[1])
							))) {
		...
		bool CServerDriver::isLookingAhead(double roll, double pitch) {
			return (((g_isHoldingAccuracyButton && !flipButtonUse)
					|| (!g_isHoldingAccuracyButton && flipButtonUse)) || (roll > (_standardRollForward - _maxRollDown) && roll < (_standardRollForward + _maxRollUp)));
		}

		bool CServerDriver::isShakingHead(vr::HmdVector3d_t vel, vr::HmdVector3d_t threshold) {
